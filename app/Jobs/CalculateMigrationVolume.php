...
use App\Models\MigrationVolume;
...
/**
 * Job to calculate the migration volume for given reference geometry and year.
 */
class CalculateMigrationVolume implements ShouldQueue
{
            ...
            ->chunk(200, function ($referenceGeometries) {
                ...
                foreach ($referenceGeometries as $feature) {
                    try {
                        ...
                        $migrationVolume = $inflows + $outflows;

                        $now = now();
                        $resultsToInsert[] = [
                            'name' => 'migration_volume',
                            'value' => $migrationVolume,
                            'year' => $this->year,
                ...
                DB::transaction(function () use ($resultsToInsert) {
                    MigrationVolume::insert($resultsToInsert);
                });
            });
    }
}
